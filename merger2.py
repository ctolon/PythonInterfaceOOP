...
class D(object):
  ...
  def __init__(self, parserD=argparse.ArgumentParser(add_help=False)):
    super(D, self).__init__()
    self.parserD = parserD

  def addArguments(self):
    self.parserD.add_argument("-td", "--test-d", help="Test D", type=str, metavar="")
    #Add more arguments specific to B

  def parseArgs(self):
    return self.parserD.parse_args()

class C(object):

  def __init__(self, parserC=argparse.ArgumentParser(add_help=False)):
    super(C, self).__init__()
    self.parserC = parserC

  def addArguments(self):
    self.parserC.add_argument("-tcx", "--testx-c", help="Tesxt C", type=str, metavar="")
    #Add more arguments specific to B

  def parseArgs(self):
    return self.parserC.parse_args()
...
class A(object):
  ...
  def __init__(self, parserA=argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    description="Arguments to pass"), b=B(), c=C(), d=D()):
