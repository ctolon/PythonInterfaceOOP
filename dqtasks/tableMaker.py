...
class TableMaker(object):
    ...
    def addArguments(self):
        ...
        with open("tempCutsLibrary.h") as f:
            for line in f:
                ...
                for i in stringIfSearch:
                    getAnalysisCuts = re.findall(
                        '"([^"]*)"', i
                    )  # get in double quotes string value with regex exp.
                    getPairCuts = [y for y in getAnalysisCuts if "pair" in y]  # get pair cuts
                    if getPairCuts:  # if pair cut list is not empty
                        allPairCuts = (
                            allPairCuts + getPairCuts
                        )  # Get Only pair cuts from CutsLibrary.h
                        namespacedPairCuts = [x + oneColon for x in allPairCuts]  # paircut:
                    allAnalysisCuts = (
                        allAnalysisCuts + getAnalysisCuts
                    )  # Get all Cuts from CutsLibrary.h
                    nameSpacedallAnalysisCuts = [x + oneColon for x in allAnalysisCuts]  # cut:
                    nameSpacedallAnalysisCutsTwoDots = [
                        x + doubleColon for x in allAnalysisCuts
                    ]  # cut::
        ...
        groupTableMakerConfigs = self.parserTableMaker.add_argument_group(
            title="Data processor options: table-maker"
        )
        ...
        groupProcessTableMaker = self.parserTableMaker.add_argument_group(
            title="Data processor options: table-maker"
        )
        ...
        groupDQTrackBarrelTask = self.parserTableMaker.add_argument_group(
            title="Data processor options: d-q-track barrel-task"
        )
        groupDQTrackBarrelTask.add_argument(
            "--isBarrelSelectionTiny",
            help="Run barrel track selection instead of normal(process func. for barrel selection must be true)",
            action="store",
            type=str.lower,
            choices=booleanSelections,
        ).completer = ChoicesCompleter(booleanSelections)
        ...
        # d-q muons-selection
        groupDQMuonsSelection = self.parserTableMaker.add_argument_group(
            title="Data processor options: d-q muons-selection"
        )
        groupDQMuonsSelection.add_argument(
            "--cfgMuonsCuts",
            help="Space separated list of ADDITIONAL muon track cuts",
            action="store",
            nargs="*",
            type=str,
            metavar="CFGMUONSCUT",
        ...
        # d-q-filter-p-p-task
        groupDQFilterPP = self.parserTableMaker.add_argument_group(
            title="Data processor options: d-q-filter-p-p-task"
        )
        groupDQFilterPP.add_argument(
            ...
            type=str,
            nargs="*",
            metavar="CFGBARRELSELS",
            choices=allSels,
        ).completer = ChoicesCompleterList(allSels)
        groupDQFilterPP.add_argument(
            "--cfgMuonSels",
            help="Configure Muon Selection <muon-cut>:[<pair-cut>]:<n> example muonQualityCuts:pairNoCut:1",
            action="store",
            type=str,
            nargs="*",
            metavar="CFGMUONSELS",
        ...
        groupDQFilterPP.add_argument(
            "--isFilterPPTiny",
            help="Run filter tiny task instead of normal (processFilterPP must be true) ",
            action="store",
            type=str.lower,
            choices=booleanSelections,
        ).completer = ChoicesCompleter(booleanSelections)
        ...
        # analysis-qvector
        groupAnalysisQvector = self.parserTableMaker.add_argument_group(
            title="Data processor options: analysis-qvector"
        )
        groupAnalysisQvector.add_argument(
            "--cfgCutPtMin",
            help="Minimal pT for tracks",
            action="store",
            type=str,
            metavar="CFGCUTPTMIN",
        )
        ...
        groupAnalysisQvector.add_argument(
            "--cfgEtaLimit",
            help="Eta gap separation, only if using subEvents",
            action="store",
            type=str,
            metavar="CFGETALIMIT",
        )
        groupAnalysisQvector.add_argument(
            "--cfgNPow",
            help="Power of weights for Q vector",
            action="store",
            type=str,
            metavar="CFGNPOW",
        )
        groupAnalysisQvector.add_argument(
            "--cfgEfficiency", help="CCDB path to efficiency object", action="store", type=str
        )
        groupAnalysisQvector.add_argument(
            "--cfgAcceptance", help="CCDB path to acceptance object", action="store", type=str
        )
        ...
        groupQASelections.add_argument(
            "--cfgWithQA",
            help="If true, fill QA histograms",
            action="store",
            type=str.lower,
            choices=booleanSelections,
        ).completer = ChoicesCompleter(booleanSelections)
