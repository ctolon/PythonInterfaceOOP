...
class EMEfficiency(object):
    ...
    def __init__(self, parserEMEfficiency = argparse.ArgumentParser(formatter_class = argparse.ArgumentDefaultsHelpFormatter, description = "Example Usage: ./runEMEfficiency.py <yourConfig.json> --arg value "), helperOptions = HelperOptions(), dplAodReader = DplAodReader(), dqLibGetter = DQLibGetter()):
    ...
    def addArguments(self):
        ...
        groupAnalysisSelections = self.parserEMEfficiency.add_argument_group(title = "Data processor options: analysis-event-selection, analysis-track-selection")
        ...
        groupProcessSEPSelections = self.parserEMEfficiency.add_argument_group(title = "Data processor options: analysis-same-event-pairing")
        groupProcessSEPSelections.add_argument("--cfgBarrelMCSignals", help = "Space separated list of MC signals", nargs = "*", action = "store", type = str, metavar = "CFGBARRELMCSIGNALS", choices = allMCSignals,).completer = ChoicesCompleterList(allMCSignals)
        ...
        groupProcess = self.parserEMEfficiency.add_argument_group(title = "Choice List for analysis-same-event-pairing PROCESS_SWITCH options")
        
        for key, value in sameEventPairingProcessSelections.items():
            groupProcess.add_argument(key, help = value, action = "none")
        
        # cfg for QA
        groupQASelections = self.parserEMEfficiency.add_argument_group(title = "Data processor options: analysis-event-selection, analysis-track-selection")
        ...
        groupDPLReader = self.parserEMEfficiency.add_argument_group(title = "Data processor options: internal-dpl-aod-reader, internal-dpl-aod-writer")
        ...
        #groupDPLReader.add_argument("--writer", help = "Argument for producing dileptonAOD.root. Set false for disable", action = "store", type = str)
