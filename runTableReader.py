...
class RunTableReader(object):
    ...
    def addArguments(self):
        ...
        # Core Part
        groupCoreSelections = self.parserRunTableReader.add_argument_group(
        ...
        # aod
        groupDPLReader = self.parserRunTableReader.add_argument_group(
        ...
        # automation params
        groupAutomations = self.parserRunTableReader.add_argument_group(
        ...
        for value, value2 in value.items():
            ...
            if args.mixing is None:
                if value == "processBarrelSkimmed" and args.analysis:
                    ...
                    if key == "analysis-event-mixing":
                        ...
                        if "eventMixing" not in analysisCfg and args.onlySelect == "true":
                            config[key][value] = "false"
                            logging.debug(" - [%s] %s : false", key, value)
                        if "eventMixing" in analysisCfg and (
                            "trackSelection" not in analysisCfg
                            and "muonSelection" not in analysisCfg
                        ):
                            logging.error(
                ...
                if value == "processBarrelMuonSkimmed" and args.analysis:
                    ...
                    if key == "analysis-event-mixing":
                        if (
                            "trackSelection" in analysisCfg
                            and "muonSelection" in analysisCfg
                            and "eventMixing" in analysisCfg
                        ):
                            config[key][value] = "true"
                            logging.debug(" - [%s] %s : true", key, value)
                        if "eventMixing" not in analysisCfg and args.onlySelect == "true":
                ...
                if value == "processBarrelVnSkimmed" and args.analysis:
                    ...
                    if key == "analysis-event-mixing":
                        ...
                        if "eventMixingVn" not in analysisCfg and args.onlySelect == "true":
                            config[key][value] = "false"
                            logging.debug(" - [%s] %s : false", key, value)
                        if "eventMixingVn" in analysisCfg and (
                            "trackSelection" not in analysisCfg
                            and "muonSelection" not in analysisCfg
                        ):
                            logging.error(
            ...
            if args.mixing is not None:
                if value == "processBarrelSkimmed" and args.analysis:
                    ...
                    if key == "analysis-event-mixing":
                        if (
                            "trackSelection" in analysisCfg
                            and "eventMixing" in analysisCfg
                            and "Barrel" in mixingCfg
                        ):
                            config[key][value] = "true"
                            logging.debug(" - [%s] %s : true", key, value)
                        if (
                            "trackSelection" in analysisCfg
                            and "Barrel" not in mixingCfg
                ...
                if value == "processMuonSkimmed" and args.analysis:
                    ...
                    if key == "analysis-event-mixing":
                        if (
                            "muonSelection" in analysisCfg
                            and "eventMixing" in analysisCfg
                            and "Muon" in mixingCfg
                        ):
                            config[key][value] = "true"
                            logging.debug(" - [%s] %s : true", key, value)
                        if (
                            "muonSelection" in analysisCfg
                            and "Muon" not in mixingCfg
                            and args.onlySelect == "true"
                        ):
                            config[key][value] = "false"
                            logging.debug(" - [%s] %s : false", key, value)
                ...
                if value == "processBarrelMuonSkimmed" and args.analysis:
                    ...
                    if key == "analysis-event-mixing":
                        ...
                        if (
                            "trackSelection" in analysisCfg
                            and "muonSelection" in analysisCfg
                            and "BarrelMuon" not in mixingCfg
                            and args.onlySelect == "true"
                        ):
                            config[key][value] = "false"
                            logging.debug(" - [%s] %s : false", key, value)
                        if "eventMixing" not in analysisCfg and "BarrelMuon" in mixingCfg:
                            ...
                            sys.exit()
                        if "BarrelMuon" in mixingCfg and (
                            "muonSelection" not in analysisCfg
                            or "trackSelection" not in analysisCfg
                        ):
                            logging.error(
                ...
                if value == "processBarrelVnSkimmed" and args.analysis:
                    ...
                    if key == "analysis-event-mixing":
                        if (
                            "trackSelection" in analysisCfg
                            and "eventMixingVn" in analysisCfg
                            and "BarrelVn" in mixingCfg
                        ):
                            config[key][value] = "true"
                            logging.debug(" - [%s] %s : true", key, value)
                        if (
                            "trackSelection" in analysisCfg
                            and "BarrelVn" not in mixingCfg
                            and args.onlySelect == "true"
                        ):
                            config[key][value] = "false"
                            logging.debug(" - [%s] %s : false", key, value)
                ...
                if value == "processMuonVnSkimmed" and args.analysis:
                    ...
                    if key == "analysis-event-mixing":
                        if (
                            "muonSelection" in analysisCfg
                            and "eventMixingVn" in analysisCfg
                            and "MuonVn" in mixingCfg
                        ):
                            config[key][value] = "true"
                            logging.debug(" - [%s] %s : true", key, value)
                        if (
                            "muonSelection" in analysisCfg
                            and "MuonVn" not in mixingCfg
                            and args.onlySelect == "true"
                        ):
                            config[key][value] = "false"
                            logging.debug(" - [%s] %s : false", key, value)
            ...
            if key == "analysis-same-event-pairing" and args.process:
                ...
                if "VnJpsiToEE" in processCfg and value == "processVnJpsiToEESkimmed":
                    ...
                    if not isAnalysisTrackSelected:
                        logging.error(
                            "trackSelection not found in analysis for processVnJpsiToEESkimmed -> analysis-same-event-pairing"
                        )
                        sys.exit()
                if (
                    "VnJpsiToEE" not in processCfg
                    and value == "processVnJpsiToEESkimmed"
                    and args.onlySelect == "true"
                ):
                ...
                if "VnJpsiToMuMu" in processCfg and value == "processVnJpsiToMuMuSkimmed":
                    ...
                    if not isAnalysisMuonSelected:
                        logging.error(
                            "muonSelection not found in analysis for processVnJpsiToMuMuSkimmed -> analysis-same-event-pairing"
                        )
                        sys.exit()
                if (
                    "VnJpsiToMuMu" not in processCfg
                    and value == "processVnJpsiToMuMuSkimmed"
                    and args.onlySelect == "true"
                ):
                ...
                if "ElectronMuon" in processCfg and value == "processElectronMuonSkimmed":
                    if isAnalysisTrackSelected and isAnalysisMuonSelected:
                        ...
                        sys.exit()
                if (
                    "ElectronMuon" not in processCfg
                    and value == "processElectronMuonSkimmed"
                    and args.onlySelect == "true"
                ):
                ...
                if "All" in processCfg and value == "processAllSkimmed":
                    if (
                        isAnalysisEventSelected
                        and isAnalysisMuonSelected
                        and isAnalysisTrackSelected
                    ):
                        ...
                        sys.exit()
                if (
                    "All" not in processCfg
                    and value == "processAllSkimmed"
                    and args.onlySelect == "true"
                ):
